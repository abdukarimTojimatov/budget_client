...
import { ADD_LOAN_PAYMENT } from "../graphql/mutations/loan.mutation";
import { GET_LOAN } from "../graphql/queries/loan.query";
...
const LoanRepaymentModal = ({ isOpen, onClose, loanId }) => {
  const [addLoanPayment, { loading: createLoading }] = useMutation(ADD_LOAN_PAYMENT);
  const { data: loanData, loading: loanLoading } = useQuery(GET_LOAN, {
    variables: { id: loanId },
    skip: !loanId,
  ...
  const validateForm = () => {
    ...
    } else if (loanData?.getLoan && parseFloat(formData.amount) > loanData.getLoan.remainingAmount) {
      newErrors.amount = "To'lov miqdori qolgan qarzdan ko'p bo'lishi mumkin emas";
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await createLoanRepayment({
        variables: { input: loanRepaymentData },
        refetchQueries: ["GetLoan", "GetLoans", "GetLoansStatistics", "GetLoanRepayments"],
      });
        ...
        {loanLoading ? (
          ...
          <form className="w-full max-w-2xl flex flex-col gap-6 p-6 mx-auto" onSubmit={handleSubmit}>
            ...
            <div className="bg-gray-800/60 rounded-lg p-4 border border-gray-700/50">
              <h3 className="text-white text-lg font-medium mb-2">{loan.name}</h3>
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  <span className="text-gray-400">Qarzdor:</span>
                  <span className="text-white ml-2">{loan.borrower}</span>
                </div>
                <div>
                ...
                <div>
                  ...
                  <span className="text-white ml-2">
                    {loan.status === "active" ? "Aktiv" : 
                     ...
                     loan.status === "late" ? "Kechiktirilgan" : "Defolt"}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="relative">
                <input
                  className={`appearance-none block w-full bg-gray-800/50 text-white border border-gray-600 rounded-lg py-3 pl-4 pr-10 leading-tight focus:outline-none focus:border-green-500 ${errors.amount ? "border-red-500" : ""}`}
                  id="amount"
                  name="amount"
                  type="number"
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="relative">
                <DatePicker
                  ...
                  name="paymentDate"
                  id="paymentDate"
                  className={`appearance-none block w-full bg-gray-800/50 border border-gray-600 text-white rounded-lg py-3 px-4 leading-tight focus:outline-none focus:border-green-500 ${errors.paymentDate ? "border-red-500" : ""}`}
          ...
          <div className="py-12 text-center text-white">
            <p>Kredit ma'lumotlari topilmadi</p>
          </div>
        )}
      </div>
    </div>
